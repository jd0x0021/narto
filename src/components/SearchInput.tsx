...
export default function SearchInput() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { query: currentQuery, resolvedCommand: currentCommand } =
  ...
  const hasValidCommand =
    rawInput.trimStart().startsWith("/meme ") ||
    rawInput.trimStart().startsWith("/gif ");
  const displayCommand = hasValidCommand
    ? rawInput.trimStart().split(" ")[0]
    : "";
    ...
    <div className="shrink-0">
      ...
      >
        <div className="w-full flex items-center text-base">
          ...
          {hasValidCommand && (
            <div className="flex-1 whitespace-pre pointer-events-none text-narto-text">
              {displayQuery || (
                <span className="text-narto-muted/50">
                  Search {resolvedCommand}s...
                </span>
              )}
            </div>
          )}
        ...
        <div className="pl-4 ml-auto border-l border-narto-border text-xs text-narto-muted font-bold tracking-wider whitespace-nowrap opacity-60 flex items-center shrink-0">
          POWERED BY{" "}
          <span className="text-narto-accent ml-2 flex items-center gap-1">
            ⚡ CHCKN
          </span>
        </div>
      </div>
